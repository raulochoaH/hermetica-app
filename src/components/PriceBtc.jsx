...
const PriceBtc = () => {
  ...
  return (
    <ResponsiveContainer width={500} height={300}>
      <LineChart data={data} isAnimationActive={false} margin={{ top: 20, right: 30, left: 0, bottom: 5 }} >
        <CartesianGrid strokeWidth={0} />
        <XAxis dataKey="name" stroke="#fff" strokeWidth={1} tickLine={false} tick={{ fill: "#fff", fontSize: 11 }} />
        <YAxis domain={[minY, maxY]} stroke="#fff" tick={false} tickLine={false} axisLine={false} />
        <Tooltip formatter={(value) => parseFloat(value).toLocaleString(undefined, { minimumFractionDigits: 2 }) + " USD"}
